// MeterM1.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(MeterM1, CStatic)
...
BEGIN_MESSAGE_MAP(MeterM1, CStatic)
ON_WM_PAINT()
ON_WM_SIZE()
END_MESSAGE_MAP()

// MeterM1 message handlers
...
void MeterM1::ReconstructControl()
{
...
void MeterM1::OnSize(UINT nType, int cx, int cy) 
{
...
void MeterM1::DrawNeedle(CDC *pDC)
{
	int nResult;
...
void MeterM1::DrawValue(CDC *pDC)
{
	int nHeight;
	CPoint pttemp;
	CString strtemp;
...
void MeterM1::UpdateNeedle(double dValue)
{
	m_dCurrentValue = dValue;
	Invalidate();
}

void MeterM1::SetNeedleColor(COLORREF colorNeedle)
{
...
void MeterM1::SetRange(double dMin, double dMax)
{
	m_dMaxValue = dMax ;
	m_dMinValue = dMin ;
	ReconstructControl() ;
...
void MeterM1::SetScaleDecimals(int nDecimals)
{
	m_nScaleDecimals = nDecimals ;
	ReconstructControl() ;
}

void MeterM1::SetUnits(CString &strUnits)
{
	m_strUnits = strUnits ;
	ReconstructControl() ;
}

void MeterM1::SetValueDecimals(int nDecimals)
{
...
void MeterM1::SetColorTick(BOOL bColorTick) //改变仪表盘刻度的颜色
{
