// MeterM5.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(MeterM5, CStatic)
...
BEGIN_MESSAGE_MAP(MeterM5, CStatic)
ON_WM_PAINT()
ON_WM_SIZE()
END_MESSAGE_MAP()

// MeterM5 message handlers

void MeterM5::OnPaint() 
{
...
void MeterM5::ReconstructControl()
{
...
void MeterM5::OnSize(UINT nType, int cx, int cy) 
{
...
void MeterM5::DrawNeedle(CDC *pDC)
{
	int nResult;
...
void MeterM5::DrawValue(CDC *pDC)
{
	int nHeight;
	CPoint pttemp;
	CString strtemp;
...
void MeterM5::UpdateNeedle(double dValue)
{
	m_dCurrentValue = dValue;
	Invalidate();
}

void MeterM5::SetNeedleColor(COLORREF colorNeedle)
{
...
void MeterM5::SetRange(double dMin, double dMax)
{
	m_dMaxValue = dMax ;
	m_dMinValue = dMin ;
	ReconstructControl() ;
...
void MeterM5::SetScaleDecimals(int nDecimals)
{
	m_nScaleDecimals = nDecimals ;
	ReconstructControl() ;
}

void MeterM5::SetUnits(CString &strUnits)
{
	m_strUnits = strUnits ;
	ReconstructControl() ;
}

void MeterM5::SetValueDecimals(int nDecimals)
{
...
void MeterM5::SetColorTick(BOOL bColorTick) //改变仪表盘刻度的颜色
{
