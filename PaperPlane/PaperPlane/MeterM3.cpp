// MeterM3.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(MeterM3, CStatic)
...
BEGIN_MESSAGE_MAP(MeterM3, CStatic)
ON_WM_PAINT()
ON_WM_SIZE()
END_MESSAGE_MAP()

// MeterM3 message handlers

void MeterM3::OnPaint() 
{
...
void MeterM3::DrawMeterBackground(CDC *pDC, CRect &rect)
{
...
void MeterM3::ReconstructControl()
{
...
void MeterM3::OnSize(UINT nType, int cx, int cy) 
{
...
void MeterM3::DrawNeedle(CDC *pDC)
{
	int nResult;
...
void MeterM3::DrawValue(CDC *pDC)
{
	int nHeight;
	CPoint pttemp;
	CString strtemp;
...
void MeterM3::UpdateNeedle(double dValue)
{
	m_dCurrentValue = dValue;
	Invalidate();
}

void MeterM3::SetNeedleColor(COLORREF colorNeedle)
{
...
void MeterM3::SetRange(double dMin, double dMax)
{
	m_dMaxValue = dMax ;
	m_dMinValue = dMin ;
	ReconstructControl() ;
...
void MeterM3::SetScaleDecimals(int nDecimals)
{
	m_nScaleDecimals = nDecimals ;
	ReconstructControl() ;
}

void MeterM3::SetUnits(CString &strUnits)
{
	m_strUnits = strUnits ;
	ReconstructControl() ;
}

void MeterM3::SetValueDecimals(int nDecimals)
{
...
void MeterM3::SetColorTick(BOOL bColorTick) //改变仪表盘刻度的颜色
{
