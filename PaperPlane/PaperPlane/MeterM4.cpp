// MeterM4.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(MeterM4, CStatic)
...
BEGIN_MESSAGE_MAP(MeterM4, CStatic)
ON_WM_PAINT()
ON_WM_SIZE()
END_MESSAGE_MAP()

// MeterM4 message handlers

void MeterM4::OnPaint() 
{
...
void MeterM4::DrawMeterBackground(CDC *pDC, CRect &rect)
{
...
void MeterM4::ReconstructControl()
{
...
void MeterM4::OnSize(UINT nType, int cx, int cy) 
{
...
void MeterM4::DrawNeedle(CDC *pDC)
{
	int nResult;
...
void MeterM4::DrawValue(CDC *pDC)
{
	int nHeight;
	CPoint pttemp;
	CString strtemp;
...
void MeterM4::UpdateNeedle(double dValue)
{
	m_dCurrentValue = dValue;
	Invalidate();
}

void MeterM4::SetNeedleColor(COLORREF colorNeedle)
{
...
void MeterM4::SetRange(double dMin, double dMax)
{
	m_dMaxValue = dMax ;
	m_dMinValue = dMin ;
	ReconstructControl() ;
...
void MeterM4::SetScaleDecimals(int nDecimals)
{
	m_nScaleDecimals = nDecimals ;
	ReconstructControl() ;
}

void MeterM4::SetUnits(CString &strUnits)
{
	m_strUnits = strUnits ;
	ReconstructControl() ;
}

void MeterM4::SetValueDecimals(int nDecimals)
{
...
void MeterM4::SetColorTick(BOOL bColorTick) //改变仪表盘刻度的颜色
{
