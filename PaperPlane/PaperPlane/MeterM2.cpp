// MeterM2.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(MeterM2, CStatic)
...
BEGIN_MESSAGE_MAP(MeterM2, CStatic)
ON_WM_PAINT()
ON_WM_SIZE()
END_MESSAGE_MAP()

// MeterM2 message handlers

void MeterM2::OnPaint() 
{
...
void MeterM2::DrawMeterBackground(CDC *pDC, CRect &rect)
{
...
void MeterM2::ReconstructControl()
{
...
void MeterM2::OnSize(UINT nType, int cx, int cy) 
{
...
void MeterM2::DrawNeedle(CDC *pDC)
{
	int nResult;
...
void MeterM2::DrawValue(CDC *pDC)
{
	int nHeight;
	CPoint pttemp;
	CString strtemp;
...
//	strtemp.Format("%.*lf", m_nValueDecimals, m_dCurrentValue); ///////////////////////////////////yingmou
	...
	m_font.CreateFont (nHeight, 0, 0, 0, 400,
						...
						CLIP_DEFAULT_PRECIS,
						DEFAULT_QUALITY, 
						DEFAULT_PITCH|FF_SWISS,_T( "Arial")) ;//改变仪表盘文字格式
	pFontOld = pDC->SelectObject(&m_font);
...
void MeterM2::UpdateNeedle(double dValue)
{
	m_dCurrentValue = dValue;
	Invalidate();
}

void MeterM2::SetNeedleColor(COLORREF colorNeedle)
{
...
void MeterM2::SetRange(double dMin, double dMax)
{
	m_dMaxValue = dMax ;
	m_dMinValue = dMin ;
	ReconstructControl() ;
...
void MeterM2::SetScaleDecimals(int nDecimals)
{
	m_nScaleDecimals = nDecimals ;
	ReconstructControl() ;
}

void MeterM2::SetUnits(CString &strUnits)
{
	m_strUnits = strUnits ;
	ReconstructControl() ;
}

void MeterM2::SetValueDecimals(int nDecimals)
{
...
void MeterM2::SetColorTick(BOOL bColorTick) //改变仪表盘刻度的颜色
{
